function base.create_item_on_point(id, target)
    ---@ui 在~2~处创建物品~1~
    ---@belong item
    ---@description 创建物品
    ---@applicable both
    ---@arg1 base.unit_get_point(e.unit)
    local item = base.item.create_to_point(id, target, target:get_scene())
    base.last_created_item = item
    return item
end
 ---@keyword 创建 点
function base.create_item_on_unit(id, target)
    ---@ui 创建物品~1~并交给~2~
    ---@belong item
    ---@description 为单位创建物品
    ---@applicable both
    ...
    if unit_check(target) then
        local item = base.item.create_to_unit(id, target)
        base.last_created_item = item
        return item
    end
end
 ---@keyword 创建 单位
function base.unit_add_item(unit, item)
    ---@ui 为~1~添加物品~2~
    ---@belong item
    ...
    ---@arg2 e.unit
    if (unit_check(unit) and item_check(item)) then
        return item:add_to(unit)
    end
end
 ---@keyword 添加 单位
function base.unit_has_item(unit, id)
    ---@ui 单位~1~是否持有物品~2~
    ---@belong item
 ...
 ---@keyword 持有 单位
function base.unit_all_items(unit)
    ---@ui 获取~1~身上所有物品
    ---@belong item
 ...
 ---@keyword 获取
function base.item_add_extra_mod(item, buff_id_name, IsEquip)
 ...
 ---@keyword 词条
function base.remove_extra_mod(item, buff_id_name, IsEquip)
 ...
 ---@keyword 词条
function base.item_generate_rand_mod(item)
    ---@ui 为~1~生成词条随机结果
    ---@belong item
 ...
 ---@keyword 词条
function base.get_last_created_item()
    ---@ui 触发器最后创建的物品
    ---@belong item
 ...
 ---@keyword 物品
function base.item_rnd_value(item, buff_id_name, prop_name)
 ...
 ---@keyword 词条
function base.item_set_stack(item, stack)
    ---@ui 设置~1~的堆叠层数为~2~
    ---@belong item
 ...
 ---@keyword 词条
function base.item_stack(item)
    ---@ui ~1~的堆叠层数
    ---@belong item
    ---@description 物品的堆叠层数
    ---@applicable value
    if item_check(item) then
        return (item.stack or 0)
    end
end
 ---@keyword 单位
function base.item_unit(item)
    ---@ui 物品~1~的物品单位
    ---@belong item
 ...
 ---@keyword 单位
function base.item_unit_get_item(unit)
    ---@ui 物品单位~1~对应的物品对象
    ---@belong item
 ...
 ---@keyword 单位
function base.item_blink(item, target)
    ---@ui 将物品~1~瞬移到到点~2~
    ---@belong item
    ---@description 移动物品
    ---@applicable action
 ...
 ---@keyword 移动
function base.item_get_holder(item)
    ---@ui 物品~1~的持有者单位
    ---@belong item
 ...
 ---@keyword 持有者 单位
function base.item_get_name(item)
    ---@ui 物品~1~的Id
    ---@belong item
    ---@description 物品的Id
    ---@applicable value
 ...
 ---@keyword 物品 Id
function base.item_grant_tag(item)
    ---@ui 物品~1~被赋予的标签
    ---@belong item
    ---@description 物品被赋予的标签
    ---@applicable value
    if (item_check(item) and item.granted_tag) then
        return item.granted_tag
    end
    return ''
end
 ---@keyword 标签
function base.item_get_owner(item)
    ---@ui 物品~1~的拥有者玩家
    ---@belong item
 ...
 ---@keyword 持有者 玩家
function base.item_remove(item)
    ---@ui 移除物品~1~
    ---@belong item
    ---@description 移除物品
    ---@applicable action
...
function base.item_skill(item)
 ...
 ---@keyword 技能
function base.item_get_equip_state(item)
    ---@ui 物品~1~是否在装备状态
    ---@belong item
    ---@description 物品的装备状态
    ---@applicable value
    if item_check(item) then
        local slot_cache = (item.slot and item.slot.cache)
 ...
 ---@keyword 物品
function base.get_inventory_items(unit, index)
 ...
 ---@keyword 物品
function base.give_item_to_inventory(item, unit, index)
