function base.player_add_attribute(player, state, value)
    ---@ui 使~1~的属性~2~增加~3~
    ---@belong player
    ---@description 玩家增加属性
    ---@applicable action
 ...
 ---@keyword 属性 增加
function base.get_player_controller(player)
    ---@ui ~1~的控制者类型
    ---@belong player
...
function base.player_game_state(player)
    ---@ui ~1~的游戏状态
    ---@belong player
    ---@description 玩家的游戏状态
    ---@applicable value
 ...
 ---@keyword 游戏状态
function base.player_get_attribute(player, state)
    ---@ui ~1~的~2~属性
    ---@belong player
    ---@description 玩家的属性
    ---@applicable value
 ...
 ---@keyword 属性
function base.player_get_hero(player)
    ---@ui ~1~的主控单位
    ---@belong player
    ---@description 玩家的主控单位
    ---@applicable value
 ...
 ---@keyword 主控单位
function base.player_get_slot_id(player)
    ---@ui ~1~的槽位Id
    ---@belong player
    ---@description 玩家的槽位Id
    ---@applicable value
 ...
 ---@keyword 槽位
function base.player_get_team_id(player)
    ---@ui ~1~的队伍Id
    ---@belong player
    ---@description 玩家的队伍Id
    ---@applicable value
...
function base.get_player_input_rocker(player)
    ---@ui ~1~的摇杆方向
    ---@belong player
    ---@description 玩家的摇杆方向
    ---@applicable value
...
function base.is_player_abort(player)
    ---@ui ~1~是否已放弃游戏
    ---@belong player
 ...
 ---@keyword 放弃
function base.kick_player(player, backend, frontend)
 ...
 ---@keyword 踢出
function base.player_leave_reason(player)
    ---@ui 玩家的~1~退出记录
    ---@belong player
    ---@description 玩家的退出记录
    ---@applicable value
 ...
 ---@keyword 退出
function base.player_send_message(player, text, type, time)
 ...
 ---@keyword 玩家 消息
function base.player_message_box(player, text)
    ---@ui 向~1~发送弹框消息:~2~
    ---@belong player
 ...
 ---@keyword 玩家 消息
function base.player_set_attribute_number(player, state, value)
    ---@ui 将~1~的属性~2~设置为~3~
    ---@belong player
 ...
 ---@keyword 设置 属性
function base.player_set_attribute_string(player, state, value)
    ---@ui 将~1~的属性~2~设置为~3~
    ---@belong player
 ...
 ---@keyword 设置 属性
function base.player_set_afk(player)
    ---@ui 将~1~设置为挂机状态
    ---@belong player
 ...
 ---@keyword 设置 状态
function base.player_set_hero(player, hero)
    ---@ui 设置~1~的主控单位为~2~
    ---@belong player
 ...
 ---@keyword 设置 单位
function base.player_set_team_id(player, id)
    ---@ui 设置~1~的队伍Id为~2~
    ---@belong player
 ...
 ---@keyword 设置 队伍
function base.get_player_user_agent(player)
    ---@ui ~1~的用户客户端
    ---@belong player
 ...
 ---@keyword 客户端
function base.player_user_id(player)
    ---@ui ~1~的虚拟用户Id
    ---@belong player
 ...
 ---@keyword 用户 Id
function base.player_get_scene_name(player)
    ---@ui ~1~所在场景的名称
    ---@belong player
 ...
 ---@keyword 场景
function base.player_get_user_nick(player)
    ---@ui 玩家~1~的昵称
    ---@belong player
    ---@description 玩家的昵称
    ---@applicable value
 ...
 ---@keyword 昵称
function base.get_each_player(type)
    ---@ui 获取~1~玩家类型所有玩家
    ---@belong player
    ...
    ---@name1 玩家类型
    local players = {}
    for player in base.each_player(type) do
