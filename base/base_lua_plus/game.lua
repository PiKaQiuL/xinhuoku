...
function base.player_jump_scene(player, scene, keep_hero)
 ...
 ---@keyword 跳转 场景
function base.game_ui_message(message_name, data)
    ---@ui 向客户端发送消息~1~,数据为~2~
    ---@belong game
    ---@description 向客户端发消息
    ---@applicable action
...
local player_info = {}
function base.player_win_game(player)
    ---@ui ~1~赢得游戏
    ---@belong player
    ---@description 玩家赢得游戏
    ---@applicable action
    ---@name1 玩家
    ---@arg1 base.player(1)
    if (player_check(player) and player_info[player] == nil) then
...
function base.player_fail_game(player)
    ---@ui ~1~游戏失败
    ---@belong player
    ---@description 玩家游戏失败
    ---@applicable action
...
function base.object_store_value(object, key, value)
    ---@ui 在~1~上保存值：~3~，索引为~2~
    ---@belong game
...
function base.object_restore_value(object, key)
    ---@ui ~1~上保存的索引为~2~的值
    ---@belong game
...
function base.pause_game_time(sec)
    ---@ui 暂停游戏~1~秒
    ---@belong game
...
function base.unpause_game()
 ...
 ---@keyword 停止 取消
function base.switch_fov_mode(number, scene)
    ---@ui 切换场景~2~迷雾模式为~1~
    ---@belong game
    ---@description 切换迷雾模式
