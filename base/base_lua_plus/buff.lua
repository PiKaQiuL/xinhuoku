...
function base.get_last_created_buff()
    ---@ui 触发器最后创建的Buff
    ---@belong buff
...
function base.unit_add_buff(unit, buff_id_name, stack)
    ...
    if unit_check(unit) then
        local buff = unit:add_buff_new(buff_id_name, stack)
...
function base.buff_set_stack(buff, count)
    ---@ui 设置Buff~1~的层数为~2~
    ---@belong buff
 ...
 ---@keyword 设置 层数
function base.buff_get_pulse(buff)
    ---@ui ~1~的周期
    ---@belong buff
    ---@description Buff周期
    ---@applicable value
 ...
 ---@keyword 周期
function base.buff_get_remaining(buff)
    ---@ui Buff~1~的剩余时间
    ---@belong buff
 ...
 ---@keyword 时间
function base.buff_get_stack_all(unit, link)
    ---@ui 单位~1~身上Buff~2~的总层数（计算所有实例）
    ---@belong buff
    ---@description 指定Id的buff的总层数（计算所有实例）
    ---@applicable value
    local c = 0
    for buff in unit:each_buff(link) do
        c = c + (buff:get_stack() or 0)
    end
    return c
end
 ---@keyword 层数
function base.buff_get_stack(buff)
    ---@ui Buff~1~的层数
    ---@belong buff
    ---@description Buff层数
    ---@applicable value
    ---@arg1 base.get_last_created_buff()
    if buff_check(buff) then
        return (buff:get_stack() or 0)
    else
        return 0
    end
end
 ---@keyword 层数
function base.buff_remove(buff)
    ---@ui 移除Buff~1~
    ---@belong buff
    ---@description 移除Buff
    ---@applicable action
 ...
 ---@keyword 移除
function base.buff_set_pulse(buff, pulse)
 ...
 ---@keyword 设置 周期
function base.buff_set_remaining(buff, remaining)
 ...
 ---@keyword 设置 时间
function base.unit_each_buff(unit, id)
 ...
 ---@keyword 获取 Id
function base.unit_find_buff(unit, id)
 ...
 ---@keyword 获取 Id
function base.unit_has_buff(unit, id)
 ...
 ---@keyword 获取 类型
function base.buff_get_id(buff)
    ---@ui Buff~1~的Id
    ---@belong buff
    ---@description Buff的Id
    ---@applicable value
 ...
 ---@keyword Id
function base.buff_get_level(buff)
    ---@ui Buff~1~的等级
    ---@belong buff
    ---@description Buff的等级
    ---@applicable value
 ...
 ---@keyword 等级
function base.buff_set_level(buff, level)
    ---@ui 设置Buff~1~的等级为~2~
    ---@belong buff
 ...
 ---@keyword 等级
function base.buff_get_tracked_units(buff)
    ---@ui 获取Buff~1~追踪的所有单位
    ---@belong buff
