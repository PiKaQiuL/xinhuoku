function base.unit_set_loot(unit, link)
    ---@ui 设置~1~的击杀奖励列表为~2~
    ---@belong unit
...
function base.get_last_created_unit()
    ---@ui 触发器最后创建的单位
    ---@belong unit
...
function base.player_create_unit(player, id, where, face)
    ...
    ---@name4 朝向
    ---@arg1 base.player(1)
    if (player_check(player) and point_check(where)) then
        local unit = player:create_unit(id, where, face, nil, where:get_scene())
 ...
 ---@keyword 创建 单位
function base.player_create_unit_ai(player, id, where, face, default_ai)
    ...
    ---@arg1 "否"
    ---@arg2 base.player(1)
    local unit = base.player_create_unit(player, id, where, face)
    if (default_ai) then
 ...
 ---@keyword 创建 单位
function base.player_create_unit_on_scene(player, id, where, face, scene)
    ...
    if player_check(player) then
        local unit = player:create_unit(id, where, face, nil, scene)
        base.last_created_unit = unit
        if not(unit) then
            log.debug'单位创建失败，请检查创建点是否可用且单位Id是否正确'
        end
 ...
 ---@keyword 创建 单位
function base.player_create_unit_illusion(player, unit, where, face)
    ...
    ---@name4 朝向
    ---@arg1 base.player(1)
    if (player_check(player) and unit_check(unit) and point_check(where)) then
        local new_unit = player:create_illusion(where, face, unit, nil, where:get_scene())
 ...
 ---@keyword 创建 镜像
function base.player_create_unit_illusion_on_scene(player, unit, where, face, scene)
    ...
    ---@arg1 base.player(1)
    if player_check(player) then
        local new_unit = player:create_illusion(where, face, unit)
        if not(new_unit) then
            log.debug'单位镜像创建失败，请检查创建点是否可用且单位Id是否正确'
            base.last_created_unit = nil
            return
 ...
 ---@keyword 创建 镜像
function base.unit_create_unit_illusion(unit, dest, where, face)
    ...
    ---@name3 创建位置
    ---@name4 朝向
    if (unit_check(unit) and unit_check(dest) and point_check(where)) then
        local result = unit:create_illusion(where, face, dest, nil, where:get_scene())
 ...
 ---@keyword 创建 镜像
function base.create_unit_illusion(unit, where, face)
    ...
    ---@name3 朝向
    if unit_check(unit) then
        local result = unit:create_illusion(where, face)
        base.last_created_unit = result
        return result
    end
    base.last_created_unit = nil
 ...
 ---@keyword 创建 镜像
function base.unit_get_id(unit)
    ---@ui ~1~的编号
    ---@belong unit
    ---@description 单位的编号
    ---@applicable value
 ...
 ---@keyword 编号
function base.unit_add_attribute(unit, state, value)
 ...
 ---@keyword 修改 属性
function base.unit_add_attribute_ex(unit, state, value, value_type)
 ...
 ---@keyword 修改 属性
function base.unit_add_ai(unit, name, data)
...
function base.unit_play_animation(unit, name, speed, loop, part)
 ...
 ---@keyword 播放 动画
function base.unit_add_height(unit, height)
    ---@ui 提高~1~的高度~2~
    ---@belong unit
    ---@description 增加单位高度
    ---@applicable action
 ...
 ---@keyword 增加 高度
function base.unit_add_provide_sight(unit, team)
    ---@ui 使~1~提供视野给队伍~2~
    ---@belong unit
    ---@description 共享单位视野
    ---@applicable action
 ...
 ---@keyword 队伍 视野
function base.unit_add_resource(unit, energy_type, value)
    ---@ui 使~1~的能量-~2~增加~3~
    ---@belong unit
    ---@description 增加单位能量
    ---@applicable action
...
function base.unit_add_mark(unit, unit_type)
    ---@ui 为~1~添加标记~2~
    ---@belong unit
    ---@description 为单位添加标记
    ---@applicable action
 ...
 ---@keyword 添加 标记
function base.unit_add_sight(unit, sight)
    ---@ui 为~1~添加可见形状~2~
    ---@belong unit
...
-- TODO 多返回值
function base.unit_get_attribute(unit, state)
    ---@ui ~1~的属性~2~
    ---@belong unit
 ...
 ---@keyword 属性
function base.unit_get_attribute_ex(unit, state, value_type)
    ---@ui ~1~的属性~2~的~3~值
    ---@belong unit
    ---@description 单位的属性
    ---@applicable value
 ...
 ---@keyword 属性
function base.unit_get_attribute_max(unit, state)
    ---@ui ~1~的属性~2~的上限
    ---@belong unit
 ...
 ---@keyword 属性 上限
function base.unit_get_attribute_min(unit, state)
    ---@ui ~1~的属性~2~的下限
    ---@belong unit
 ...
 ---@keyword 属性 下限
function base.unit_get_class(unit)
    ---@ui ~1~的类别
    ---@belong unit
    ---@description 单位的类别
    ---@applicable value
...
function base.unit_get_facing(unit)
    ---@ui ~1~的朝向
    ---@belong unit
    ---@description 单位的朝向
    ---@applicable value
 ...
 ---@keyword 角度 朝向
function base.unit_get_height(unit)
    ---@ui ~1~的离地高度
    ---@belong unit
    ---@description 单位的离地高度
    ---@applicable value
 ...
 ---@keyword 高度
function base.unit_get_creation_param(unit)
    ---@ui 创建了~1~的效果节点
    ---@belong unit
 ...
 ---@keyword 高度
function base.unit_get_name(unit)
    ---@ui 获取~1~的Id
    ---@belong unit
    ---@description 单位的Id
    ---@applicable value
 ...
 ---@keyword 名字
function base.unit_get_player(unit)
    ---@ui ~1~的所属玩家
    ---@belong unit
    ---@description 单位的所属玩家
    ---@applicable value
 ...
 ---@keyword 玩家
function base.unit_set_player(unit, player)
    ---@ui 设置~1~的所属玩家为~2~
    ---@belong unit
    ...
    ---@arg1 base.player(1)
    ---@arg2 e.unit
    if (unit_check(unit) and player_check(player)) then
        unit:set_owner(player) --实际是owner
    end
end
 ---@keyword 玩家
function base.unit_get_point(unit)
    ---@ui ~1~的坐标
    ---@belong unit
    ---@description 单位的坐标
    ---@applicable value
 ...
 ---@keyword 坐标
function base.unit_get_resource(unit, resource_type)
    ---@ui ~1~的能量-~2~的值
    ---@belong unit
...
function base.unit_get_mark(unit, unit_mark)
    ---@ui ~1~的行为标记~2~的计数
    ---@belong unit
 ...
 ---@keyword 标记 层数
function base.unit_get_attackable_radius(unit)
    ---@ui ~1~的选取半径
    ---@belong unit
    ---@description 单位的选取半径
    ---@applicable value
 ...
 ---@keyword 选取半径
function base.unit_get_team_id(unit)
    ---@ui ~1~的队伍Id
    ---@belong unit
    ---@description 单位的队伍Id
    ---@applicable value
 ...
 ---@keyword 队伍 Id
function base.unit_get_tag(unit)
    ---@ui ~1~的标签
    ---@belong unit
    ---@description 单位的标签
    ---@applicable value
 ...
 ---@keyword 标签
function base.unit_get_walk_command_a(unit)
    ---@ui ~1~的移动理由
    ---@belong unit
    ---@description 单位的移动理由
    ---@applicable value
 ...
 ---@keyword 移动 理由
function base.unit_get_walk_command_b_point(unit)
    ---@ui ~1~的移动目标点
    ---@belong unit
    ...
    ---@arg1 e.unit
    if unit_check(unit) then
        local _, target = unit:get_walk_command()
        if target then
 ...
 ---@keyword 移动 目标
function base.unit_get_walk_command_b_unit(unit)
    ---@ui ~1~的移动目标单位
    ---@belong unit
 ...
 ---@keyword 移动 目标
function base.unit_walk(unit, target)
    ---@ui 令~1~向~2~移动
    ---@belong unit
    ---@description 令单位向点移动
    ---@applicable action
...
function base.unit_has_mark(unit, unit_mark)
    ---@ui ~1~是否存在标记~2~
    ---@belong unit
 ...
 ---@keyword 标记
function base.unit_is_alive(unit)
    ---@ui ~1~是否存活
    ---@belong unit
    ---@description 单位是否存活
    ---@applicable value
 ...
 ---@keyword 存活 死亡
function base.unit_is_ally_of_unit(unit, dest)
    ---@ui ~1~是否是~2~的友方
    ---@belong unit
 ...
 ---@keyword 盟友 友方
function base.unit_is_ally_of_player(unit, dest)
    ---@ui ~1~是否是~2~的友方
    ---@belong unit
 ...
 ---@keyword 盟友 友方
function base.unit_is_enemy_of_unit(unit, dest)
    ---@ui ~1~是否是~2~的敌人
    ---@belong unit
    ...
    ---@name2 目标单位
    ---@arg1 e.unit
    if (unit_check(unit) and unit_check(dest)) then
        return unit:is_enemy(dest)
    end
end
 ---@keyword 敌对 敌人
function base.unit_is_enemy_of_player(unit, dest)
    ---@ui ~1~是否是~2~的敌人
    ---@belong unit
    ...
    ---@name2 目标玩家
    ---@arg1 e.unit
    if (unit_check(unit) and player_check(dest)) then
        return unit:is_enemy(dest)
    end
end
 ---@keyword 敌对 敌人
function base.unit_is_illusion(unit)
    ---@ui ~1~是否是镜像
    ---@belong unit
    ---@description 单位是否是镜像
    ---@applicable value
 ...
 ---@keyword 镜像
function base.unit_is_in_range_of_unit(unit, target, radius)
    ---@ui ~1~是否在~2~的~3~距离内
    ---@belong unit
 ...
 ---@keyword 单位 距离
function base.unit_is_in_range_of_point(unit, target, radius)
    ---@ui ~1~是否在点~2~的~3~距离内
    ---@belong unit
 ...
 ---@keyword 点 距离
function base.unit_is_visible_to_unit(unit, target)
    ---@ui ~1~是否对~2~可见
    ---@belong unit
 ...
 ---@keyword 可见 视野
function base.unit_is_visible_to_player(unit, target)
    ---@ui ~1~是否对~2~可见
    ---@belong unit
 ...
 ---@keyword 可见 视野
function base.unit_is_walking(unit)
    ---@ui ~1~是否在移动
    ---@belong unit
    ---@description 单位是否在移动
    ---@applicable value
 ...
 ---@keyword 移动
function base.unit_add_z_speed(unit, speed)
    ---@ui 使~1~的Z轴速度值增加~2~
    ---@belong unit
 ...
 ---@keyword 增加 速度
function base.unit_set_z_speed(unit, speed)
    ---@ui 设置~1~的Z轴速度值为~2~
    ---@belong unit
 ...
 ---@keyword 设置 速度
function base.unit_get_z_speed(unit)
    ---@ui ~1~的Z轴速度值
    ---@belong unit
 ...
 ---@keyword Z 速度
function base.unit_kill(unit, killer)
    ---@ui 杀死~1~，并设置凶手为~2~
    ---@belong unit
    ---@description 杀死单位
    ---@applicable action
 ...
 ---@keyword 杀死 凶手
function base.unit_learn_skill(unit, skill_id_name)
    ---@ui 令~1~学习技能：~2~
    ---@belong unit
    ---@description 令单位学习技能
    ---@applicable action
...
function base.unit_reborn(unit, where)
    ---@ui 使~1~在~2~处复活
    ---@belong unit
    ---@description 复活单位
    ---@applicable action
    ...
    ---@name2 复活位置
    ---@arg1 e.unit
    if (unit_check(unit) and point_check(where)) then
 ...
 ---@keyword 复活
function base.unit_remove(unit)
    ---@ui 移除~1~
    ---@belong unit
    ---@description 移除单位
    ---@applicable action
 ...
 ---@keyword 移除
function base.unit_remove_animation(unit, animation_name)
    ---@belong unit
    ---@description 移除单位的动画
    ---@applicable action
    ---@name1 单位
 ...
 ---@keyword 移除 动画
function base.unit_remove_buff(unit, buff_name)
    ---@ui 移除~1~身上的~2~Buff
    ---@belong unit
 ...
 ---@keyword 移除 Buff
function base.unit_remove_privide_sight(unit, team_id)
    ---@ui 移除~1~对队伍~2~的视野共享
    ---@belong unit
...
function base.unit_remove_mark(unit, unit_mark)
    ---@ui 移除~1~身上的行为标记~2~
    ---@belong unit
 ...
 ---@keyword 移除 标记
function base.unit_replace_skill(unit, skill_id_old, skill_id_new)
 ...
 ---@keyword 替换 技能
-- 仅限内置属性
function base.unit_set(unit, state, value)
    ---@ui 设置~1~的属性~2~为~3~
    ---@belong unit
 ...
 ---@keyword 属性
function base.unit_set_ex(unit, state, value, value_type)
 ...
 ---@keyword 属性
function base.unit_set_str(unit, state, value)
 ...
 ---@keyword 属性
function base.unit_set_attribute_max(unit, state, value)
 ...
 ---@keyword 属性 上限
function base.unit_set_attribute_min(unit, state, value)
 ...
 ---@keyword 属性 下限
function base.unit_set_attribute_sync(unit, state, sync)
 ...
 ---@keyword 属性 同步方式
function base.unit_set_facing(unit, facing)
    ---@ui 设置~1~的朝向为~2~度
    ---@belong unit
    ---@description 设置单位的朝向
    ---@applicable action
 ...
 ---@keyword 朝向
function base.unit_set_height(unit, height)
    ---@ui 设置~1~的高度为~2~
    ---@belong unit
    ---@description 设置单位高度
    ---@applicable action
 ...
 ---@keyword 高度
function base.unit_set_model(unit, model)
    ---@ui 设置~1~的模型为~2~
    ---@belong unit
    ---@description 设置单位模型
    ---@applicable action
 ...
 ---@keyword 模型
function base.unit_set_resource(unit, energy_type, value)
    ---@ui 设置~1~的能量-~2~为~3~
    ---@belong unit
    ---@description 设置单位能量
    ---@applicable action
 ...
 ---@keyword 能量
function base.unit_set_attackable_radius(unit, radius)
    ---@ui 设置~1~的选取半径为~2~
    ---@belong unit
...
function base.unit_texttag(unit, target, text, text_type, sync, r, g, b, size)
 ...
 ---@keyword 文字 UI
function base.unit_get_scene_name(unit)
    ---@ui ~1~所在场景的名称
    ---@belong unit
 ...
 ---@keyword 场景 名称
function base.unit_jump_scene(unit, scene_name, position)
    ...
    ---@arg1 e.unit
    if unit_check(unit) then
        local success = unit:jump_scene(scene_name)
        if success then
            unit:blink(position)
        end
        return success
 ...
 ---@keyword 场景 移动
function base.unit_jump_scene2(unit, position)
    ---@ui 将~1~转移到~2~处
    ---@belong unit
    ---@description 转移单位场景
    ---@applicable action
    ...
    ---@arg1 e.unit
    if unit_check(unit) then
        local success = unit:jump_scene(position:get_scene())
        if success then
            unit:blink(position)
        end
        return success
...
function base.node_mark(node_mark, unit_name)
    return node_mark
end

function base.set_location_async(unit, position)
    ---@ui 异步设置~1~单位的位置到~2~
    ---@belong unit
 ...
 ---@keyword 单位 移动
function base.set_facing_async(unit, facing)
    ---@ui 异步设置~1~单位的朝向为~2~
    ---@belong unit
 ...
 ---@keyword 单位 朝向
function base.unit_get_exp(unit)
    ---@ui ~1~的经验值
    ---@belong unit
    ---@description 单位的经验值
    ---@applicable value
 ...
 ---@keyword 经验
function base.unit_add_exp(unit, exp, ignore_fraction)
 ...
 ---@keyword 增加 经验
function base.unit_set_exp(unit, exp)
    ---@ui 设置~1~的经验值为~2~
    ---@belong unit
    ---@description 设置单位经验值
    ---@applicable action
 ...
 ---@keyword 设置 经验
function base.unit_get_level(unit)
    ---@ui ~1~的等级
    ---@belong unit
    ---@description 单位的等级
    ---@applicable value
 ...
 ---@keyword 等级
function base.unit_add_level(unit, level)
    ---@ui 使~1~的等级提高~2~级
    ---@belong unit
    ---@description 提高单位等级
    ---@applicable action
 ...
 ---@keyword 增加 等级
function base.unit_set_level(unit, level)
    ---@ui 设置~1~的等级为~2~
    ---@belong unit
    ---@description 设置单位等级
    ---@applicable action
 ...
 ---@keyword 等级
function base.unit_get_max_level(unit)
    ---@ui ~1~的等级上限
    ---@belong unit
    ---@description 单位的等级上限
    ---@applicable value
 ...
 ---@keyword 等级上限
function base.unit_set_max_level(unit, max_level)
    ---@ui 设置~1~的等级上限为~2~
    ---@belong unit
 ...
 ---@keyword 等级上限
function base.unit_get_single_level_exp(unit, level)
    ---@ui ~1~的第~2~级所需的经验
    ---@belong unit
 ...
 ---@keyword 等级 经验
function base.unit_get_cumu_level_exp(unit, level)
    ---@ui ~1~升到~2~级总共所需的经验
    ---@belong unit
 ...
 ---@keyword 等级 经验
function base.unit_get_exp_fraction(unit)
    ---@ui ~1~的经验倍率
    ---@belong unit
    ---@description 单位的经验倍率
    ---@applicable value
 ...
 ---@keyword 经验倍率
function base.unit_set_exp_fraction(unit, fraction)
    ---@ui 设置~1~的经验倍率为~2~
    ---@belong unit
 ...
 ---@keyword 经验倍率
function base.unit_set_prohibit_exp_distribute(unit, value)
 ...
 ---@keyword 经验 分配
function base.unit_set_level_profile(unit, profile_id)
    ---@ui 设置~1~的升级配置为~2~
    ---@belong unit
 ...
 ---@keyword 升级配置
function base.unit_grant_loot(unit, target, link)
    ---@ui 使~1~对~2~直接给予奖励~3~
    ---@belong unit
    ---@description 直接给予单位奖励
    ---@applicable action
    if (unit_check(target) and unit_check(unit)) then
...
function base.get_unit_from_id(id)
    ---@ui 编号为~1~的单位
    ---@belong unit
