local lt2oa = require "base.mprofiler_lt2oa"
...
local function mnl2ff (len)
  if len <= 4 then
    return function (n)
      return string.format("%6d B", math.ceil( n ))
    end
  end
...
---@return stirng 
local function flat_print(func_table, max)
  ...
  for i,v in ipairs(ftarray) do
    ...
    result = result .. string.format(flfp, table.unpack(p)) .. '\n'
  end
  return result
end
...
"                                      %scalled/total%s  children",
  ...
  local result = string.format(header[1], sc, sc) .. '\n' .. string.format(header[2], sc, sc, sn) .. '\n' .. string.format(header[3], sc, sc) .. '\n' .. separator .. '\n'
  ...
  local function cglprint(ms, mc, c, oc, n, i, mp)
    ...
    if not mp then  -- parent or child
      result = result .. string.format(flcgpp, ms, mc, c, oc, adjc, n, adjn, i) .. '\n'
